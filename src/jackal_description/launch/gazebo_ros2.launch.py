...
def _run(context, *args, **kwargs):
    ...
    # 3) Inject ros2_control + joint_state_publisher plugins into the <model> block
    #    - Use absolute YAML path (SDF doesn't expand $(find ...))
    ctrl_yaml = str(get_package_share_path('jackal_control') / 'config' / 'control.yaml')

    # Confirm the plugin filename on your system (normally "gz_ros2_control-system")
    # You can check via: gz plugin --list | grep -i ros2_control
    ros2_control_plugin = 'gz_ros2_control-system'

    plugin_block = f"""
      <plugin filename="{ros2_control_plugin}" name="gz_ros2_control-system::GazeboSimROS2ControlPlugin">
      ...
      <!-- Optional, handy to get /joint_states from sim -->
      <plugin filename="gz-sim-joint-state-publisher-system" name="gz::sim::systems::JointStatePublisher"/>
    """

    # Insert plugin just before the closing </model> of the first model (your robot)
...
# 8) Spawn controllers after the manager is up (adjust manager ns if you add one)
    spawn_jsb = Node(
