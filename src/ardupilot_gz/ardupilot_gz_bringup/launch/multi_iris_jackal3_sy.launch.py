...
from launch.actions import (
    ...
    SetEnvironmentVariable, TimerAction, ExecuteProcess, GroupAction
)
...
def _prepare_and_spawn_jackal(context, *args, **kwargs):
    ...
    # 3) Robot State Publisher under /jackal
    rsp = Node(
        package='robot_state_publisher',
        executable='robot_state_publisher',
        namespace='jackal', # << namespaced
    ...
    # spawn the controller spawners inside the /jackal namespace so they look up /jackal/controller_manager
    spawn_jsb = Node(
        package='controller_manager',
        executable='spawner',
        namespace='jackal', # << run spawner in jackal namespace
...
def generate_launch_description():
    ...
    # run the iris include inside the /iris namespace to isolate its topics and TFs
    iris_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
