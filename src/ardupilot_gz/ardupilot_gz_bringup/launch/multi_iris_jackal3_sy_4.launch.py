...
def _prepare_and_spawn_jackal(context, *args, **kwargs):
    ...
    # ✅ (Edit 4) Auto-inject gz_ros2_control plugin if missing
    ...
    # ✅ (Edit 2) RSP without namespace for visible TFs
    rsp = Node(
        package='robot_state_publisher',
        executable='robot_state_publisher',
        name='robot_state_publisher',
        output='screen',
        parameters=[{'robot_description': urdf_xml.decode('utf-8'),
                     'use_sim_time': True}],
    )
    ...
    # ✅ (Edit 3) Explicit spawn logging
    spawn = ExecuteProcess(
        cmd=['ros2', 'run', 'ros_gz_sim', 'create',
             '-file', sdf_file,
             '-name', 'jackal',
             '-x', x, '-y', y, '-z', z, '-Y', yaw],
        output='both',   # print stdout + stderr
        shell=False
    )
...
def generate_launch_description():
    ...
    # ✅ (Edit 1) Increase delay to 8 seconds before spawning Jackal
