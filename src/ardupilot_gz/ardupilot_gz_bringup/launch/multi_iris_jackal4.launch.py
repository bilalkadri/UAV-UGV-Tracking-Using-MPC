...
def _prepare_and_spawn_jackal(context, *args, **kwargs):
    ...
    xacro_file = os.path.join(pkg_share, 'urdf', 'jackal3.urdf.xacro')
...
def generate_launch_description():
    ...
    args = [
        ...
        DeclareLaunchArgument("jackal_y", default_value="0.0"),
        DeclareLaunchArgument("jackal_z", default_value="1.0"),
    ...
    env = [
        ...
        # Models/resources (Jackal + ArduPilot worlds/models + Baylands Textures)
        SetEnvironmentVariable('GZ_SIM_RESOURCE_PATH', [
            # Link to Baylands textures so 'Terrain' model finds its images
            os.path.expanduser('~/.gz/fuel/fuel.gazebosim.org/openrobotics/models/baylands/tip/materials/textures'),
    ...
    rviz = Node(
        ...
        executable="rviz2",
        arguments=["-d", f'{Path(pkg_project_bringup) / "rviz" / "iris.rviz"}'],
        parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')}], # Add this
        condition=IfCondition(LaunchConfiguration("rviz")),
    )
