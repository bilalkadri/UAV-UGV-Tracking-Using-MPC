...
# Now includes proper namespacing for both /iris/ and /jackal/.
...
from launch.actions import (
    ...
    SetEnvironmentVariable, TimerAction, ExecuteProcess, GroupAction, PushRosNamespace
)
...
# -------------------------------------------------------------
# Function to prepare and spawn the Jackal UGV with /jackal namespace
# -------------------------------------------------------------
def _prepare_and_spawn_jackal(context, *args, **kwargs):
    ...
    # Inject gz_ros2_control plugin if missing
    ...
    # 3) Robot State Publisher (under /jackal namespace)
    rsp = GroupAction([
    ...
    # 4) Spawn Jackal model in Gazebo
    ...
    # 5) Start controllers under /jackal namespace
    spawn_jsb = Node(
...
def generate_launch_description():
    ...
    env = [
        ...
        SetEnvironmentVariable('GZ_SIM_SYSTEM_PLUGIN_PATH', [
            TextSubstitution(text=user_ws_lib + ':'),
    ...
    # Iris UAV with /iris namespace
    iris = GroupAction([
        PushRosNamespace('iris'),
    ...
    return LaunchDescription(
        args + env + [
            ...
            iris,  # /iris namespace
            rviz,
            # Delay Jackal spawn slightly so Gazebo finishes loading
            TimerAction(period=8.0, actions=[OpaqueFunction(function=_prepare_and_spawn_jackal)])
        ]
    )
