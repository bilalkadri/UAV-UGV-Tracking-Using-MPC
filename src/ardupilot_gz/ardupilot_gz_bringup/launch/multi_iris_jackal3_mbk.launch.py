...
def _prepare_and_spawn_jackal(context, *args, **kwargs):
        ...
        parameters=[
            {
                ...
                'use_sim_time': True,
                'frame_prefix': 'jackal/'  ### ← MODIFIED: prefix all TF frames
            }
        ],
    )
    ...
    spawn = Node(
        package='ros_gz_sim',
        executable='create',
        namespace='jackal',  ### ← MODIFIED: put under /jackal namespace
        output='screen',
        arguments=[
            '-file', sdf_file,
            '-name', 'jackal',
    ...
    # 4.5) Controller Manager Node (ros2_control_node)
    jackal_ctrl_yaml = os.path.join(pkg_share, 'config', 'control_drive.yaml')
    ...
    return [rsp, spawn, controller_manager, spawn_jsb, spawn_diff]
...
def generate_launch_description():
    ...
    iris = IncludeLaunchDescription(
        ...
        launch_arguments={
            "namespace": "iris",  ### ← MODIFIED: UAV in its own namespace
        }.items(),
    )
    # RViz.
    rviz = Node(
