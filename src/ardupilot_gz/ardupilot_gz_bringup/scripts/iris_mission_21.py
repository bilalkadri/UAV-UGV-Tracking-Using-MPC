...
import matplotlib.pyplot as plt  # ✅ Added for plotting trajectories
...
pid_z = PID(0.02, 0.0, 0.005, (-0.8, 0.8))   # forward/back

# ============================================================
# ======== GLOBAL TRAJECTORY LISTS FOR UAV & UGV =============
# ============================================================
uav_traj = []   # (x, y) UAV positions
ugv_traj = []   # (x, y) UGV positions
...
def move_jackal_forward(node, duration=5.0, speed=0.4):
    ...
    jackal_pub.publish(msg)
    node.get_logger().info("✅ Jackal stopped or duration finished.")

def follow_ugv(node, master):
    """PID-based drone following using ArUco feedback."""
    global pid_x, pid_y, pid_z, uav_traj
    print("🚁 Drone started following UGV (PID active)...")
    ...
    while TRACK_FLAG == '1':
        if node.marker_center and node.frame_center and node.marker_size:
            err_x = node.frame_center[0] - node.marker_center[0]
            err_y = node.frame_center[1] - node.marker_center[1]
            err_z = 100 - node.marker_size  # desired marker size ~100 px

            vx = pid_z.update(err_z)
            vy = pid_x.update(err_x)
            vz = -pid_y.update(err_y)

            send_velocity(master, vx, vy, vz, start_time)

            # ✅ Record approximate trajectory (integrating velocity)
            x += vx * dt
            y += vy * dt
            uav_traj.append((x, y))
...
def track_monitor_and_start(master, node):
    ...
    while rclpy.ok():
        if TRACK_FLAG == '1' and not already_started:
            node.get_logger().info("Monitor: TRACK_FLAG == '1' — starting Jackal and PID follow threads.")
            # ✅ Modified to move for 5 seconds at 0.1 m/s
            threading.Thread(target=move_jackal_forward, args=(node, 5.0, 0.1), daemon=True).start()
            threading.Thread(target=follow_ugv, args=(node, master), daemon=True).start()
            already_started = True

        if TRACK_FLAG != '1' and already_started:
            node.get_logger().info("Monitor: TRACK_FLAG cleared — will allow restart on next detection.")
            already_started = False

        time.sleep(0.1)

def plot_trajectories():
    """Plots UAV and UGV trajectories."""
...
def main():
    ...
    node.get_logger().info("➡️ Moving forward while searching for ArUco marker...")
    start_time = time.time()
    ...
    node.get_logger().info("🔶 TRACK_FLAG == '1' — ArUco marker detected and monitor will start follow threads.")
    while TRACK_FLAG == '1':
        time.sleep(0.2)
    ...
    # ✅ After landing, plot trajectories
    plot_trajectories()
