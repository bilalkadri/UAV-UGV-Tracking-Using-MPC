...
class ArucoDetector(Node):
    def __init__(self, master):
        ...
        #whenever the /camera/image topic receives an image image_callback() function is called
...
#----------------------------------------------------------------------
#  THe following function should only work when TRACK_FLAG==1
#------------------------------------------------------------------------
# THe follwoing function was written by Sofia
...
#                     print('The linear velocities vx and vy are',vx,vy)
#                     vz = 0.0
#                     yaw_rate=0.0
...
#             send_velocity(master, 0.0, 0.0, 0.0, 0.1,start_time)
...
#------------------------------------------------------------------------------
# THe following function is modified by MBK
#-------------------------------------------------------------------------------
def follow_ugv(node, master):
    """PID control loop that activates only while TRACK_FLAG == '1'.
       Option A: PID fully overrides circular motion while active."""
    ...
    while True:
       ...
       #the following if condition says that if the ARUCO Marker is detetcted then run the code
        if node.marker_center and node.frame_center:
            err_x = node.frame_center[0] - node.marker_center[0]
            err_y = node.frame_center[1] - node.marker_center[1]
            ...
            # PID outputs velocities in image-pixel space -> you may want mapping to world velocities;
            # here we use the PID outputs directly as vx, vy (small gains). vy is inverted for image coords.
            vx = -.5*pid_x.update(err_x)
            vy = .5*pid_y.update(err_y)
            print('The linear velocities vx and vy are',vx,vy)
            vz = 0.0
            yaw_rate=0.0
...
def main():
    ...
    rclpy.init()
    #node is an instance of the class ArucoDetector
    # master is the parameter being passed to the class constructor
    # as fasr as i understand , there is no reason to pass master as an argument in the constructor
    node = ArucoDetector(master)

    #----------------------------------------------------------------------
    #----------------------------------------------------------------------
    # The first thread which is detecting the ARUCO marker
    # run ROS spin in background
    # This becomes a ROS Node ((i must study this in detail ROS-1 has no such concept))
    #----------------------------------------------------------------------
    #----------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------
    # The second thread (moving the UGV on a circular trajectory)
    #----------------------------------------------------------------------
    #MBK Comments
    # The UGV Must start moving now, so  i must simply make a publisher which publishes velocity 
    # on the UGV so that UGV keeps moving in circular trajectory
    # jackal_pub = node.create_publisher(
    #     TwistStamped, '/jackal/jackal_velocity_controller/cmd_vel', 10
    # )
    # stop_event = threading.Event()
    # pub_thread = threading.Thread(target=publish_rectangle, args=(node, jackal_pub, stop_event), daemon=True)
    # pub_thread.start()
