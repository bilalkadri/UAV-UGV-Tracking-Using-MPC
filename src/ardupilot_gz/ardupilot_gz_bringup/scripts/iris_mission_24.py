...
class ArucoDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        if ids is not None and len(ids) > 0:
            ...
            self.marker_center = self.filtered_center
            cv2.circle(frame, (int(self.filtered_center[0]), int(self.filtered_center[1])), 5, (0, 255, 0), -1)
            cv2.putText(frame, f"Marker Center ({int(cx)},{int(cy)})", (10, 30),
...
# ============================================================
def move_jackal_forward(node, duration=10.0, speed=0.3):
    global ugv_traj
...
def follow_ugv(node, master):
    ...
    while TRACK_FLAG == '1':
        if node.marker_center and node.frame_center:
            ...
            vx = -pid_x.update(err_x)  # correct sign
            vy = -pid_y.update(err_y)
            vz = 0.0

            # Clamp velocities to safety
            vx = max(-0.8, min(0.8, vx))
            vy = max(-0.8, min(0.8, vy))

            send_velocity(master, vx, vy, vz, start_time)

            print(f"[PID] err_x={err_x:.2f}, err_y={err_y:.2f}, vx={vx:.3f}, vy={vy:.3f}")
...
def track_monitor_and_start(master, node):
    ...
    while rclpy.ok():
        if TRACK_FLAG == '1' and not already_started:
            ...
            threading.Thread(target=move_jackal_forward, args=(node, 10.0, 0.3), daemon=True).start()
