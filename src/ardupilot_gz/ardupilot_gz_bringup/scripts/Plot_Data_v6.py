...
class PlotterNode(Node):
    ...
    def save_data(self):
        ...
        # copy under lock to avoid concurrent modification while saving
        with self.lock:
        ...
        # save if non-empty, otherwise create empty files with headers
        try:
            if len(odom) > 0:
    ...
    def live_plot_loop(self):
        ...
        while self.run_plot:
            ...
            # Copy data under lock to local variables (minimize lock time)
            with self.lock:
            ...
            if len(local_odom) > 0:
                ...
                # ensure equal lengths (defensive)
                if len(xs) == len(ys):
            ...
            if len(local_vel_est) > 0:
                vx = [v[0] for v in local_vel_est]
                vy = [v[1] for v in local_vel_est]
                ax3.plot(vx, label="Estimated vx")
                ax3.plot(vy, label="Estimated vy")

            if len(local_mpc) > 0:
                vx_cmd = [v[0] for v in local_mpc]
                vy_cmd = [v[1] for v in local_mpc]
            ...
            if len(local_odom_vel) > 0:
                vx = [v[0] for v in local_odom_vel]
                vy = [v[1] for v in local_odom_vel]
                vz = [v[2] for v in local_odom_vel]
            ...
            if len(local_odom_ang) > 0:
                wx = [v[0] for v in local_odom_ang]
                wy = [v[1] for v in local_odom_ang]
                wz = [v[2] for v in local_odom_ang]
