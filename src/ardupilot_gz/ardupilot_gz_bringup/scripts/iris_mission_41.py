...
class VelocityEstimator(Node):
    def __init__(self, master):
        ...
        self.create_subscription(
            Odometry, '/jackal/odometry', self.jackal_odom_cb, 10
        )
    ...
    def jackal_odom_cb(self, msg: Odometry):
        ...
        self.jackal_vel = np.array([lv.x, lv.y, lv.z], dtype=float)
        self.jackal_omega = np.array([av.x, av.y, av.z], dtype=float)
...
def main():
    ...
    vel_node = VelocityEstimator(master)

    # === Spin both ArucoDetector and VelocityEstimator ===
