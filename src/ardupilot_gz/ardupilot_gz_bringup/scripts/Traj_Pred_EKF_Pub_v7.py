...
class RelativePoseEKF(Node):
    ...
    def imu_cb(self, msg):
        try:
            ...
            self.v_u = np.array([msg.linear_acceleration.x,
                                 msg.linear_acceleration.y,
                                 msg.linear_acceleration.z]) * 0.0  # change 1
        except Exception:
    ...
    def ekf_predict_publish(self):
        ...
        rel_vel =  self.v_g - self.v_u # change 2
        rel_omega =  self.omega_g - self.omega_u  # change 3

        self.x[0:3,0] += rel_vel * self.dt
        self.x[3:6,0] += rel_omega * self.dt

        #self.x[0:3,0] += rel_vel.reshape(3,1) * self.dt      # change 4
        #self.x[3:6,0] += rel_omega.reshape(3,1) * self.dt    # change 5

        self.P = (self.P + self.Q)*0.98 # add small damping (change 6)
