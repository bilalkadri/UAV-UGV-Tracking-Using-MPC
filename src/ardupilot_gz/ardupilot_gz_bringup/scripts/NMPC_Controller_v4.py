...
class SimpleMPCNode(Node):
    def __init__(self):
        super().__init__('simple_nmpc_controller_v4_node')
        ...
        self.create_subscription(TwistStamped, self.uav_odom_topic, self.cb_uav_odom, 10)
    ...
    def cb_ugv_odom(self, msg: Odometry):
        self.v_g = np.array([msg.twist.twist.linear.x,
                             msg.twist.twist.linear.y,
                             msg.twist.twist.linear.z], dtype=float)
        self.w_g = np.array([msg.twist.twist.angular.x,
                             msg.twist.twist.angular.y,
                             msg.twist.twist.angular.z], dtype=float)

    def cb_uav_odom(self, msg: TwistStamped):
        self.v_u = np.array([msg.twist.linear.x,
                             msg.twist.linear.y,
                             msg.twist.linear.z], dtype=float)
        self.w_u = np.array([msg.twist.angular.x,
                             msg.twist.angular.y,
