...
def follow_ugv(node, master):
    """PID-based drone following using ArUco feedback (X-Y only)."""
    ...
    while TRACK_FLAG == '1':
        if node.marker_center and node.frame_center:
            err_x = node.frame_center[0] - node.marker_center[0]  # horizontal
            err_y = node.frame_center[1] - node.marker_center[1]  # vertical
            err_x = -err_x
