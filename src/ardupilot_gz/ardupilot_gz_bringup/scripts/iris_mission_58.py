...
class ArucoDetector(Node):
    def __init__(self, master):
        ...
        # NEW: store latest frame for display in main thread
    ...
    def image_callback(self, msg):
        ...
        # NEW: save frame for main-thread display (instead of imshow here)
        with self.lock:
...
class MPCVelocitySubscriber(Node):
    ...
    def cmd_callback(self, msg):
        ...
        vx = msg.twist.linear.x*scale
        vy = msg.twist.linear.y*scale
...
def main():
    ...
    print("Takeoff complete. UAV hovering at target altitude.")
    ...
    # --------------------------------------------
    time.sleep(5.0)   # ensure UAV is stable after takeoff

    # ===== CREATE ROS2 NODES AFTER TAKEOFF =====
    node = ArucoDetector(master)
    ekf_node = RelativePoseEKF()
    vel_node = UAVVelocityEstimator()
    mpc_node = SimpleMPCNode()
    ...
    # optional events for other threads
    plot_stop_event = threading.Event()
    stop_event_rect = threading.Event()
    ...
    print("MPC subscriber activated — ready to receive /mpc/cmd_vel")

    # ===== MAIN LOOP (safe for OpenCV GUI) =====
    try:
        while rclpy.ok():

            # Display latest frame from ArucoDetector (main thread)
            if node.latest_frame is not None:
                with node.lock:
                    frame_to_show = node.latest_frame.copy()

                cv2.imshow("Aruco Detection", frame_to_show)
                cv2.waitKey(1)
    ...
    # ===== DESTROY NODES =====
    for n in [node, ekf_node, vel_node, mpc_node, plotter_node, mpc_subscriber]:
        try:
            n.destroy_node()
        except Exception:
            pass
