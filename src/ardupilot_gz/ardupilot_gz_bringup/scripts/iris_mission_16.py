...
def mavlink_mission():
    ...
    # store trajectory points (approx)
    x_hist, y_hist = [0.0], [0.0]
    x, y = 0.0, 0.0
    ...
    # ===== Plot the straight trajectory =====
    plt.figure()
    plt.plot(y_hist, x_hist, 'r-', linewidth=2)
    plt.title("Drone Trajectory - Straight Flight (Phase 1)")
    plt.xlabel("East (m)")
    plt.ylabel("North (m)")
    plt.axis('equal')
    plt.grid(True)
