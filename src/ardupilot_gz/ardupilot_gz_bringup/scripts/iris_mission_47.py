...
from Traj_Pred_EKF_Pub import RelativePoseEKF
...
def main():
    ...
    ekf_thread = threading.Thread(target=rclpy.spin, args=(ekf_node,), daemon=True)
    ekf_thread.start()
    ...
    # destroy aruco node and shutdown rclpy
    try:
        node.destroy_node()
    except Exception:
        pass

    # destroy the EKF Node
