...
class ArucoDetector(Node):
    def __init__(self):
        ...
        # Jackal position tracking variables
        self.jackal_x = 0.0
        self.jackal_y = 0.0
        self.jackal_yaw = 0.0
        self.x_data = [self.jackal_x]
        self.y_data = [self.jackal_y]

        # Start a background thread for live plotting
        self.plot_thread = threading.Thread(target=self.plot_jackal_trajectory, daemon=True)
    ...
    def move_jackal_forward(self, duration=5.0, speed=0.5):
        ...
        while (time.time() - start_time) < duration:
            ...
            self.jackal_x += speed * math.cos(self.jackal_yaw) * dt
            self.jackal_y += speed * math.sin(self.jackal_yaw) * dt

            self.x_data.append(self.jackal_x)
            self.y_data.append(self.jackal_y)
    ...
    def plot_jackal_trajectory(self):
        """Continuously plot Jackal trajectory in real-time."""
        plt.ion()
        fig, ax = plt.subplots()
        ax.set_title("Jackal Estimated Trajectory")
        ax.set_xlabel("X Position (m)")
        ax.set_ylabel("Y Position (m)")
        line, = ax.plot([], [], 'r-', linewidth=2, label="Jackal Path")
        ax.legend()
        ax.grid(True)
